...
"""This class is responable for the generative design. Its core is a genetic algorithm.
It should out source population create ect. It really just takes a random population and explores the space


Inputs -> A sample Population, encode, decode functions. reduce everything to the same genome.
Output -> Optimal design.


"""

class ProjectDreamCatcher():
  def __init__(self, init_popultion, encode, decode, **kwargs):

      # Init Variables
      self.isAlive = True
      self.current_pop = init_popultion
  ...
  def run(self):
      ...
      pass
  def show(self, item):
      """Manager to choose which items to display, (fitness, population, energy)"""
      pass
  def end(self):
      """Implement End case here"""
      return True
  # def crossOver():
  #
  # def mutate():
